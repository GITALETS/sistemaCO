...
def validate_rpe(rpe: str) -> str:
    """ Valida que el RPE/RPU tenga exactamente 5 caracteres """
    clean_rpe = str(rpe).strip()
    if len(clean_rpe) != 5:
        raise ValueError(f"El RPE/RPU '{clean_rpe}' debe tener exactamente 5 caracteres (ejemplo: G982P o 84729).")
    return clean_rpe
...
def fill_file_01_seguimiento(wb, data, period_info, base_dir="."):
    ...
    puesto_actual = detect_worker_type(rpe)
